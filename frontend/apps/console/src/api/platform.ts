...
export const modelConfigurationApi = {
  get: () =>
    apiClient.get<ModelConfiguration>('/control/v1/platform/model-configuration'),
  update: (payload: ModelConfigurationUpdate) =>
    apiClient.put<ModelConfiguration>('/control/v1/platform/model-configuration', payload),
  test: () =>
    apiClient.post<{ connected: boolean; inputTokens: number; outputTokens: number }>(
...
export const knowledgeApi = {
  ...
  previewRemote: (
    ...
    apiClient.post<RemoteSourcePreview>(
      `${environmentBase(applicationId, environmentId)}/collections/${collectionId}/documents/remote-preview`,
      payload,
      { timeout: 35_000 }
    ),
  ...
  refreshRemote: (applicationId: string, environmentId: string, documentId: string) =>
    apiClient.post<{ document: KnowledgeDocument; ingestionRun: IngestionRun }>(
      `${environmentBase(applicationId, environmentId)}/documents/${documentId}/refresh`,
      {}
    ),
...
export interface RemoteSourcePayload {
  title: string
  url: string
  sourceType: 'auto' | 'web' | 'api' | 'feed' | 'text'
  method: 'GET' | 'POST'
