...
AI 简历、AI 基金等业务会重复需要知识入库、检索、回答、联网数据和运行治理能力。当前项目统一建设这些公共能力，并向各业务项目提供稳定、清晰、可持续扩展的接口。
...
8. 缺少统一的领域边界和契约会增加前后端不一致与维护成本。

因此项目按统一产品模型、前端、后端模块和公开 API 建设，形成一套完整且边界清晰的能力底座。
...
第一阶段采用模块化单体：API、领域服务、Provider 和 Worker 边界清晰，但不提前拆成微服务。
...
- 仓库只有一套有效前端、后端、数据库 Schema、配置和启动入口。
- 配置、密钥、日志和业务数据按环境严格隔离。
- 所有知识通过当前入库流水线解析、切割、向量化并发布。
- 开发、测试、生产环境使用独立配置、数据和 API Key。
- 发布通过版本化镜像、数据库备份和可回退配置完成，不直接覆盖不可恢复的数据。
...
| 阶段 | 目标 | 主要交付 |
| --- | --- | --- |
| 0. 产品与契约 | 冻结领域和接口 | PRD、数据模型、OpenAPI、错误码、页面信息架构 |
...
| 7. 发布与上线 | 完成安全发布 | 初始化数据、回归验证、备份恢复、生产发布与回滚演练 |
...
1. 创建开发、测试和生产数据库与对象存储边界。
2. 执行 Alembic 到唯一 Head，并保存迁移日志。
3. 创建应用、环境、知识集合、检索策略和回答策略。
4. 通过当前入库流水线导入经过确认的原始资料。
5. 核对文档数、字符数、Chunk 数、失败数与知识归属。
6. 使用固定问题集验证检索、回答模式、引用、错误和延迟。
7. 为业务后端签发最小 Scope 的生产 API Key。
8. 备份数据库、对象存储和生产配置，构建版本化镜像。
9. 在测试环境完成同版本发布后，再发布生产环境。
10. 监控错误率、任务失败率、延迟、引用与模型兜底比例。
...
回滚时恢复上一版本镜像和配置；数据库使用发布前备份或经过验证的向下迁移，失败现场保留只读副本用于排查。不得使用删除数据的方式代替回滚。
