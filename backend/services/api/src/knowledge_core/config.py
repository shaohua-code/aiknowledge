...
def _find_project_root() -> Path:
    """从源码位置定位项目根目录，避免配置读取依赖启动命令的当前目录。"""
...
class Settings(BaseSettings):
    ...
    # 根目录提供整套项目配置；后端目录和启动目录中的 .env 可按需覆盖。
    model_config = SettingsConfigDict(
        env_file=(PROJECT_ROOT / ".env", API_ROOT / ".env", Path(".env")),
    ...
    remote_fetch_timeout_seconds: int = 20
    remote_fetch_max_bytes: int = 20 * 1024 * 1024
    remote_fetch_max_redirects: int = 5
    remote_fetch_retries: int = 2
    remote_fetch_user_agent: str = "AIKnowledgeBot/2.1 (+public-knowledge-ingestion)"
    ...
    def validate_production_secrets(self) -> Settings:
        # 根目录 .env 面向 Docker Compose，使用 postgres/redis 容器主机名。
        # Windows 上通过 CMD 直接运行 API 时，自动换成宿主机暴露的端口和本地存储。
        running_in_container = Path("/.dockerenv").exists()
        if self.app_env != "production" and not running_in_container:
            self.database_url = self.database_url.replace("@postgres:5432/", "@localhost:5433/")
            self.redis_url = self.redis_url.replace("redis://redis:6379/", "redis://localhost:6379/")
            ...
            if os.name == "nt" and self.object_storage_path.startswith("/"):
                self.object_storage_path = str(API_ROOT / "storage")
